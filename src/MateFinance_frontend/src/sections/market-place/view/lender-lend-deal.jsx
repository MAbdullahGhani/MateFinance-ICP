...
const LenderLendDeal = () => {
  ...
  async function _depositFunds() {
    ...
    const USDT = new ethers.Contract(
      check ? haqq?.usdtAddress : arbitrum?.usdtAddress,
      check ? haqq?.usdtabi : arbitrum?.usdtabi,
      deployer
    );
    const borrowerExist = await invoiceMateContract.borrowerExist(
      financialDetail?.borrower?.borrowerWallet
    );
    const borrowerData = await invoiceMateContract.getBorrowerLoanDetails(
      financialDetail?.borrower?.borrowerWallet,
      financialDetail?.returnBlockChainId
    );
    ...
    try {
      ...
      if (borrowerExist == true) {
        ...
        if (lenderBalance >= principal) {
          if (check) {
            // HAQ
            await USDT.connect(lender).approve(
  ...
  return (
    <>
      ...
      <Dialog maxWidth="lg" fullWidth open={selectWallet.value} onClose={selectWallet.onFalse}>
            ...
            <Scrollbar>
              <TableContainer sx={{ position: 'relative', overflow: 'unset' }}>
                <Table size={'medium'} sx={{ minWidth: 960 }}>
                  ...
                  <TableBody>
                    {lenderDetails?.wallet?.map((item, index) => (
                      <TableRow key={index + 1}>
                        ...
                        <TableCell>
                          <Button
                            ...
                            variant={item?._id === lendingWallet?._id ? 'contained' : 'outlined'}
                            color="primary"
                            onClick={() => {
        ...
        <DialogActions>
          <Button
            disabled={!lendingWallet}
            onClick={handleLendingReq}
            color="primary"
            variant="contained"
          >
            Submit
