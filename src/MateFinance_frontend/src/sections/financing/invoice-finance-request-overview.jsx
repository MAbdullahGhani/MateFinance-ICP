    ...
    <Grid container>
      <Grid item xs={12}>
        <List sx={{ display: 'flex', flexDirection: 'row', justifyContent: 'space-between' }}>
          ...
          <ListItem>
            <ListItemText
              primary={'Requested Loan Amount'}
              secondary={Currency(invoiceData?.principal || 0, invoiceData?.currency || '', false)}
            />
          </ListItem>
          <ListItem>
            <ListItemText
          ...
          <ListItem>
            <ListItemText
              primary={'Status'}
              secondary={
                <Label color="primary">{invoiceData?.DealStatus?.toUpperCase() || '-'}</Label>
              }
            />
          </ListItem>
        </List>
