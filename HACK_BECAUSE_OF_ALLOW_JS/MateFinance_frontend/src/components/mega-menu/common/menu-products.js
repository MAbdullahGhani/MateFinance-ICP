...
export default function MenuProducts({ products, displayProduct = 8, sx }) {
    ...
    return (<Box sx={{ position: 'relative', pt: 2, ...sx }}>
      <CarouselArrows filled onNext={carousel.onNext} onPrev={carousel.onPrev} leftButtonProps={{
            size: 'small',
        ...
        <Carousel ref={carousel.carouselRef} {...carousel.carouselSettings}>
          {products.map((product) => (<Link key={product.name} component={RouterLink} href={product.path} color="inherit" underline="none" sx={{
                px: 1,
                display: 'block',
              ...
              <Image alt={product.coverUrl} src={product.coverUrl} ratio="1/1" disabledEffect sx={{ borderRadius: 1, mb: 1 }}/>
