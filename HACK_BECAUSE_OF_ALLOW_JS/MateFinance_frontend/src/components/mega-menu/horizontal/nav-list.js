...
export default function NavList({ data, slotProps }) {
    ...
    active={active} className={active ? 'active' : ''} sx={slotProps?.rootItem}/>
      ...
      {!!data.children && (<Popover disableScrollLock open={openMenu} anchorEl={navRef.current} anchorOrigin={{
                vertical: 'bottom',
                horizontal: 'left',
            }} transformOrigin={{
                vertical: 'top',
          ...
          {singleList ? (<NavSubList data={data.children} slotProps={slotProps}/>) : (<Masonry columns={4} spacing={3} defaultColumns={4} defaultSpacing={3}>
