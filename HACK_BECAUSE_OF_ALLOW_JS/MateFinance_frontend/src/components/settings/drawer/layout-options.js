...
export default function LayoutOptions({ options, value, onChange }) {
    ...
    const renderNav = (option, selected) => {
        ...
        return (<Stack spacing={0.5} flexShrink={0} direction={option === 'horizontal' ? 'row' : 'column'} sx={{
                p: 0.5,
                width: 28,
                height: 1,
    ...
    const renderContent = (selected) => (<Box sx={{ p: 0.5, flexGrow: 1, height: 1, width: 1 }}>
      <Box sx={{
            width: 1,
            height: 1,
            opacity: 0.08,
    ...
    return (<Stack direction="row" spacing={2}>
      {options.map((option) => {
            ...
            return (<ButtonBase key={option} onClick={() => onChange(option)} sx={{
                    p: 0,
                    width: 1,
                    height: 56,
                    borderRadius: 1,
