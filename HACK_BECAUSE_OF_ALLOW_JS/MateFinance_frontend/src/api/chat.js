...
export async function sendMessage(conversationId, messageData) {
     ...
     * Work in local
     */
    mutate(CONVERSATION_URL, (currentData) => {
     ...
     * Work in local
     */
    mutate(CONVERSATIONS_URL, (currentData) => {
...
export async function clickConversation(conversationId) {
    ...
    mutate([
        ...
        const conversations = currentData.conversations.map((conversation) => conversation.id === conversationId ? { ...conversation, unreadCount: 0 } : conversation);
        return {
            ...currentData,
            conversations,
        };
