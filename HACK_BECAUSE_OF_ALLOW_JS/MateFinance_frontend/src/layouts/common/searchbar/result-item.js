...
export default function ResultItem({ title, path, groupLabel, onClickItem }) {
    return (<ListItemButton onClick={onClickItem} sx={{
      ...
      <ListItemText primaryTypographyProps={{
        ...
        }} secondaryTypographyProps={{ typography: 'caption' }} primary={title.map((part, index) => (<Box key={index} component="span" sx={{
                color: part.highlight ? 'primary.main' : 'text.primary',
            }}>
            {part.text}
          </Box>))} secondary={path.map((part, index) => (<Box key={index} component="span" sx={{
