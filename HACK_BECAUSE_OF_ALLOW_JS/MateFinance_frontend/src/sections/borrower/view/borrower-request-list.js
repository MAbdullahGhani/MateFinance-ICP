...
import LenderRequestTableRow from '../borrower-request-table-row';
// ----------------------------------------------------------------------
...
// ----------------------------------------------------------------------
export default function BorrowerRequestList() {
    ...
    return (<>
      <Container maxWidth={settings.themeStretch ? false : 'lg'}>
        <CustomBreadcrumbs heading="Lending Requests" links={[
            {
                name: 'Dashboard',
        ...
        <Card>
          <Tabs value={filters.status} onChange={handleFilterStatus} sx={{
            ...
            {STATUS_OPTIONS.map((tab) => (<Tab key={tab.value} iconPosition="end" value={tab.value} label={tab.label} icon={<Label variant={((tab.value === 'all' || tab.value === filters.status) && 'filled') || 'soft'} color={(tab.value === 'completed' && 'success') ||
          ...
          <TableContainer sx={{ position: 'relative', overflow: 'unset' }}>
            ...
            .map((row) => (<LenderRequestTableRow key={row.id} row={row} selected={table.selected.includes(row.id)} onSelectRow={() => table.onSelectRow(row.id)} onDeleteRow={() => handleDeleteRow(row.id)} onViewRow={() => handleViewRow(row.id)}/>))}
...
function applyFilter({ inputData, comparator, filters, dateError }) {
    ...
    if (name) {
        inputData = inputData.filter((order) => order.orderNumber.toLowerCase().indexOf(name.toLowerCase()) !== -1 ||
