...
export default function BookingCustomerReviews({ title, subheader, list, ...other }) {
    ...
    return (<Card {...other}>
      <CardHeader title={title} subheader={subheader} action={<CarouselArrows onNext={carousel.onNext} onPrev={carousel.onPrev}/>}/>
...
function ReviewItem({ item }) {
    ...
    return (<Stack spacing={2} sx={{
      ...
      <Stack direction="row" alignItems="center" spacing={2}>
        ...
        <ListItemText primary={name} secondary={`Posted ${fDateTime(postedAt)}`} secondaryTypographyProps={{
            component: 'span',
            typography: 'caption',
            mt: 0.5,
            color: 'text.disabled',
      ...
      <Stack direction="row" flexWrap="wrap" spacing={1}>
        {tags.map((tag) => (<Chip size="small" variant="soft" key={tag} label={tag}/>))}
      </Stack>
    </Stack>);
}
