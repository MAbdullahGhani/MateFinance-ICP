...
function BankingRecentTransitionsRow({ row }) {
    ...
    const renderAvatar = (<Box sx={{ position: 'relative', mr: 2 }}>
      <Badge overlap="circular" color={row.type === 'Income' ? 'success' : 'error'} anchorOrigin={{ vertical: 'bottom', horizontal: 'right' }} badgeContent={<Iconify icon={row.type === 'Income'
                ? 'eva:diagonal-arrow-left-down-fill'
                : 'eva:diagonal-arrow-right-up-fill'} width={16}/>} sx={{
            [`& .${badgeClasses.badge}`]: {
                p: 0,
                width: 20,
            },
        }}>
        <Avatar src={row.avatarUrl || ''} sx={{
            width: 48,
            height: 48,
    ...
    return (<>
      <TableRow>
        ...
        <TableCell>
          <ListItemText primary={fDate(new Date(row.date))} secondary={fTime(new Date(row.date))} primaryTypographyProps={{ typography: 'body2' }} secondaryTypographyProps={{
            mt: 0.5,
            component: 'span',
            typography: 'caption',
        }}/>
        ...
        <TableCell>
          <Label variant={lightMode ? 'soft' : 'filled'} color={(row.status === 'completed' && 'success') ||
