...
function ProductItem({ product }) {
    ...
    return (<Stack direction="row" spacing={2}>
      ...
      <ListItemText primary={<Link sx={{ color: 'text.primary', typography: 'subtitle2' }}>{name}</Link>} secondary={<>
            {!!priceSale && (<Box component="span" sx={{ textDecoration: 'line-through', mr: 0.5 }}>
                {fCurrency(priceSale)}
              </Box>)}
