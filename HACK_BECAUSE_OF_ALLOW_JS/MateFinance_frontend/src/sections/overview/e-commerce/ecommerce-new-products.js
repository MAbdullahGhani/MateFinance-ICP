...
export default function EcommerceNewProducts({ list, ...other }) {
    ...
    return (<Card {...other}>
      <Carousel {...carousel.carouselSettings}>
        {list.map((item) => (<CarouselItem key={item.id} item={item}/>))}
      </Carousel>
    </Card>);
}
...
function CarouselItem({ item }) {
    ...
    const renderImg = (<Image alt={name} src={coverUrl} overlay={`linear-gradient(to bottom, ${alpha(theme.palette.grey[900], 0)} 0%, ${theme.palette.grey[900]} 75%)`} sx={{
            width: 1,
