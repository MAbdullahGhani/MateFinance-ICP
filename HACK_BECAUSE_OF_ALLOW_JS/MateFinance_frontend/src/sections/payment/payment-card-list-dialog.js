...
export default function PaymentCardListDialog({ open, list, onClose, selected, onSelect }) {
    ...
    const renderList = (<Stack spacing={2.5} sx={{ p: 3 }}>
      {list.map((card) => (<PaymentCardItem key={card.id} card={card} onClick={() => handleSelectCard(card)} sx={{
                cursor: 'pointer',
                ...(selected(card.id) && {
    ...
    return (<Dialog fullWidth maxWidth="xs" open={open} onClose={onClose}>
      ...
      <Stack sx={{ px: 3 }}>
        <TextField value={searchCard} onChange={handleSearchAddress} placeholder="Search..." InputProps={{
