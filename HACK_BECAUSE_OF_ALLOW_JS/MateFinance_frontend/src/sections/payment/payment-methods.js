...
export default function PaymentMethods() {
    ...
    return (<>
      <Stack spacing={5}>
        ...
        <Stack spacing={3}>
          {PAYMENT_OPTIONS.map((option) => (<OptionItem key={option.label} option={option} selected={method === option.value} isCredit={option.value === 'credit' && method === 'credit'} onOpen={newCard.onTrue} onClick={() => handleChangeMethod(option.value)}/>))}
        </Stack>
      </Stack>
...
function OptionItem({ option, selected, isCredit, onOpen, ...other }) {
    ...
    return (<Paper variant="outlined" key={value} sx={{
      ...
      <ListItemText primary={<Stack direction="row" alignItems="center">
            <Iconify icon={selected ? 'eva:checkmark-circle-2-fill' : 'eva:radio-button-off-fill'} width={24} sx={{
                mr: 2,
