...
export default function UserListView() {
    ...
    return (<>
      <Container maxWidth={settings.themeStretch ? false : 'lg'}>
        <CustomBreadcrumbs heading="List" links={[
            ...
            { name: 'List' },
        ]} action={<Button component={RouterLink} href={paths.dashboard.user.new} variant="contained" startIcon={<Iconify icon="mingcute:add-line"/>}>
              New User
            </Button>} sx={{
            mb: { xs: 3, md: 5 },
        }}/>
        ...
        <Card>
          <Tabs value={filters.status} onChange={handleFilterStatus} sx={{
            ...
            {STATUS_OPTIONS.map((tab) => (<Tab key={tab.value} iconPosition="end" value={tab.value} label={tab.label} icon={<Label variant={((tab.value === 'all' || tab.value === filters.status) && 'filled') || 'soft'} color={(tab.value === 'active' && 'success') ||
    ...
    roleOptions={_roles}/>
          ...
          {canReset && (<UserTableFiltersResult filters={filters} onFilters={handleFilters} 
        //
        ...
        results={dataFiltered.length} sx={{ p: 2.5, pt: 0 }}/>)}
          ...
          <TableContainer sx={{ position: 'relative', overflow: 'unset' }}>
            ...
            .map((row) => (<UserTableRow key={row.id} row={row} selected={table.selected.includes(row.id)} onSelectRow={() => table.onSelectRow(row.id)} onDeleteRow={() => handleDeleteRow(row.id)} onEditRow={() => handleEditRow(row.id)}/>))}
