...
export default function ProfileFollowers({ followers }) {
    ...
    return (<>
      ...
      <Box gap={3} display="grid" gridTemplateColumns={{
        ...
        {followers.map((follower) => (<FollowerItem key={follower.id} follower={follower} selected={followed.includes(follower.id)} onSelected={() => handleClick(follower.id)}/>))}
      </Box>
    </>);
}
...
function FollowerItem({ follower, selected, onSelected }) {
    ...
    return (<Card sx={{
      ...
      <Button size="small" variant={selected ? 'text' : 'outlined'} color={selected ? 'success' : 'inherit'} startIcon={selected ? <Iconify width={18} icon="eva:checkmark-fill" sx={{ mr: -0.75 }}/> : null} onClick={onSelected} sx={{ flexShrink: 0, ml: 1.5 }}>
