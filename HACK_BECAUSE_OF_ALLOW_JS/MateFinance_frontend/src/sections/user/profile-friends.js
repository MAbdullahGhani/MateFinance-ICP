...
export default function ProfileFriends({ friends, searchFriends, onSearchFriends }) {
    ...
    return (<>
      <Stack spacing={2} justifyContent="space-between" direction={{ xs: 'column', sm: 'row' }} sx={{ my: 5 }}>
        ...
        <TextField value={searchFriends} onChange={onSearchFriends} placeholder="Search friends..." InputProps={{
      ...
      {notFound ? (<SearchNotFound query={searchFriends} sx={{ mt: 10 }}/>) : (<Box gap={3} display="grid" gridTemplateColumns={{
                xs: 'repeat(1, 1fr)',
                sm: 'repeat(2, 1fr)',
                md: 'repeat(3, 1fr)',
            }}>
          {dataFiltered.map((friend) => (<FriendCard key={friend.id} friend={friend}/>))}
        </Box>)}
    </>);
}
...
function FriendCard({ friend }) {
    ...
    return (<>
      <Card sx={{
        ...
        <IconButton color={popover.open ? 'inherit' : 'default'} onClick={popover.onOpen} sx={{ top: 8, right: 8, position: 'absolute' }}>
