...
export default function OrderListView() {
    ...
    dateError={dateError}/>
          ...
          {canReset && (<OrderTableFiltersResult filters={filters} onFilters={handleFilters} 
        //
        ...
        results={dataFiltered.length} sx={{ p: 2.5, pt: 0 }}/>)}
          ...
          <TableContainer sx={{ position: 'relative', overflow: 'unset' }}>
            ...
            .map((row) => (<OrderTableRow key={row.id} row={row} selected={table.selected.includes(row.id)} onSelectRow={() => table.onSelectRow(row.id)} onDeleteRow={() => handleDeleteRow(row.id)} onViewRow={() => handleViewRow(row.id)}/>))}
