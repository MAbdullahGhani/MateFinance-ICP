...
import { Avatar, Box, Card, Container, Grid, IconButton, Paper, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Typography, } from '@mui/material';
import moment from 'moment';
import { useCallback, useEffect, useState } from 'react';
import { useNavigate } from 'react-router';
import Iconify from 'src/components/iconify/iconify';
import Scrollbar from 'src/components/scrollbar';
import { TableNoData, TablePaginationCustom, TableSkeleton, getComparator, useTable, } from 'src/components/table';
...
const FinancingProfileList = ({ setFormState, selected }) => {
    ...
    const { tableData, isLoading, error, setTableData, revalidate, isValidating } = useFinanceProfile(kpiType);
    ...
    return (<Container maxWidth="xxl">
      <div style={{ display: 'flex', justifyContent: 'center' }}>
        <SubscribersKpi subscribersKpiData={subscribersKpiData} handleKpiClick={handleKpiClick} currentKpiType={kpiType}/>
      </div>
      <Paper>
        <Card sx={{ mt: 4 }}>
          <Box sx={{ margin: '9px' }}>
            <Grid container spacing={2}>
              <Grid item xs={12} md={12} sx={{ m: -2 }}>
                <FinanceProfileTableToolbar filters={filters} onFilters={handleFilters} revalidate={revalidate} filterOptions={filtersSelect} countries={countries}/>
              </Grid>

              {canReset && (<Grid item xs={12} md={12} sx={{}}>
                  <FinanceProfileTableFiltersResult filters={filters} onFilters={handleFilters} 
        //
        ...
        {loading ? <LoadingSpinner /> : ''} */}
            ...
            dataFiltered
                ...
                .map((subscriber) => (<TableRow key={subscriber._id} hover selected={selected}>
                          <TableCell>
                          ...
                          <TableCell>
                            {`${moment(subscriber.joiningDate).format('DD-MM-YYYY')} (${daysCalculator(subscriber.joiningDate, new Date())} ${daysCalculator(subscriber.joiningDate, new Date()) === 1
                    ? 'day'
                    : 'days'})`}
                          </TableCell>
                    ...
                    : ''}
                          ...
                          <TableCell>
                            <Box style={{ display: 'flex' }}>
                              <IconButton size="small" color="primary" title="Financial Detail" onClick={() => navigate(`/admin/financing/financing-profile/finance-details/${subscriber._id}`)}>
                                <Iconify icon="ic:baseline-business-center" fontSize="small"/>
                              </IconButton>
                              <IconButton size="small" color="primary" title="View Invoices" onClick={() => navigate(`/admin/subscribers/invoices_detail/${subscriber?._id}`)}>
                                <Iconify icon="fluent:receipt-24-filled" fontSize="small"/>
                              </IconButton>
                              <IconButton color="primary" title="View Contacts" onClick={() => handleContactListOpen(subscriber.mobileNumber, `${subscriber.firstName} ${subscriber.lastName}`)}>
    ...
    dense={table.dense} onChangeDense={table.onChangeDense}/>
        ...
        <ViewContact open={showSubscriberContactModal} onClose={handleContactListClose} subscriberName={currentSubscriberNameContactModal} contactList={currentContactList} // Make sure you pass the contact list here
    />
      </Paper>
    </Container>);
};
