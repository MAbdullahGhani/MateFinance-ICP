...
function UserBiometricVerification({ financialDetail, openCommentModalHandler }) {
    ...
    return (<Card sx={{ p: 3, marginTop: 3 }}>
      <CardContent>
        ...
        {financialDetail.creditScore.biometricVerification?.status && (<Typography variant="h6" color="primary" component="h3" sx={{
                fontWeight: 600,
                textAlign: 'left',
                fontSize: '16px',
                marginTop: '10px',
        ...
        <Grid container align="center" sx={{ marginTop: '20px' }} justifyContent="space-between">
          <Grid item sm={12} md={12} lg={20}>
            {financialDetail.biometricVerificationDetial ? (<figure style={{ position: 'relative' }}>
                ...
                '&:hover, &$focusVisible': {
                  ...
                  <img src={`${userAttachments.url}${financialDetail.biometricVerificationDetial}${userAttachments.token}`} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }}/>
                  <span style={{
                position: 'absolute',
                display: 'flex',
              ...
              </figure>) : (<Typography style={{ textAlign: 'center' }}>No Detail Found</Typography>)}
          </Grid>
        </Grid>
      </CardContent>
    </Card>);
