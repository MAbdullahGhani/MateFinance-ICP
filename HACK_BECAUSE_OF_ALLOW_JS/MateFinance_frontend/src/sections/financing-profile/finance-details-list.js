...
const FinanceDetailsList = () => {
    ...
    const getCreditScore = async () => {
        try {
            ...
            const score = parseFloat(response[0].basicInfo?.score) +
    ...
    const getFinancialData = async () => {
        ...
        try {
            ...
            const user = await axios.post(CRED_HOST_API + '/users/getuser', {
    ...
    const handleExport = async () => {
        ...
        try {
            ...
            const financeResponse = await axiosInstance.post(`${endpoints.admin.exportFinanceProfileDoc}`, { _id: id }, { responseType: 'blob' });
            ...
            const invoiceResponse = await axiosInstance.post(`${endpoints.admin.downloadInvoiceFinancing}`, { _id: id }, { responseType: 'blob' });
    ...
    return (<>
      ...
      <div>
        ...
        {value !== 'requester-details' && (<UserBasicInformation financialDetail={financialDetail} subscriber={userDetail} creditScoreCriteriaList={creditScoreCriteriaList}/>)}

        {value === 'financial-details' && (<FinancialDetailsTab financialDetail={financialDetail} userDetail={userDetail} openCommentModalHandler={openCommentModalHandler} handleShowFilerDetail={handleShowFilerDetail} selectedCreditScore={selectedCreditScore} creditScoreCriteriaList={creditScoreCriteriaList}/>)}

        {value === 'requester-details' && (<RequesterDetailsTab financialDetail={financialDetail} openCommentModalHandler={openCommentModalHandler} showHistoryTableHandler={showHistoryTableHandler} selectedCreditScore={selectedCreditScore} historyLoading={historyLoading} historyList={historyList} creditScoreCriteriaList={creditScoreCriteriaList} scoreCount={scoreCount} requesterData={requesterData} 
        // userScore={userScore}
