...
export default function PostListHomeView() {
    ...
    const handleDealList = async () => {
        try {
            ...
            const response = await axiosInstance.get(`${endpoints.app.getAllDealsExt}?sortingOrder=${sortBy}&search=${searchQuery}&status=${status}`);
            setPosts(response);
    ...
    return (<>
      <Container maxWidth={settings.themeStretch ? false : 'lg'}>
        ...
        <Tabs value={status} onChange={(event, newValue) => setStatus(newValue)} sx={{
            mb: { xs: 3, md: 5 },
            mt: { xs: 3, md: 5 },
        }}>
