...
export default function PostItem({ post, index }) {
    ...
    return (<Card>
      <Box sx={{ position: 'relative' }}>
        ...
        <Avatar alt={borrower?.name} src={ATTACHMENT_URL + borrower?.logo[0]?.attachmentPath + ATTACHMENT_TOKEN} sx={{
            left: 24,
            zIndex: 9,
            bottom: -24,
            position: 'absolute',
...
export function PostContent({ title, createdAt, id, post }) {
    ...
    return (<CardContent sx={{
      ...
      <Box display="grid" alignItems="center" justifyContent="space-between" mt={1} gridTemplateColumns={{
            xs: 'repeat(1, 1fr)',
            sm: 'repeat(2, 1fr)',
        }}>
