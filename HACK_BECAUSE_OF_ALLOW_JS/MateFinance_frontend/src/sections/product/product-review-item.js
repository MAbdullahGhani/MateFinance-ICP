...
export default function ProductReviewItem({ review }) {
    ...
    const renderInfo = (<Stack spacing={2} alignItems="center" direction={{
            xs: 'row',
            md: 'column',
        }} sx={{
            width: { md: 240 },
      ...
      <ListItemText primary={name} secondary={fDate(postedAt)} primaryTypographyProps={{
            noWrap: true,
    ...
    const renderContent = (<Stack spacing={1} flexGrow={1}>
      <Rating size="small" value={rating} precision={0.1} readOnly/>

      {isPurchased && (<Stack direction="row" alignItems="center" sx={{
                color: 'success.main',
                typography: 'caption',
            }}>
      ...
      {!!attachments?.length && (<Stack direction="row" flexWrap="wrap" spacing={1} sx={{ pt: 1 }}>
          {attachments.map((attachment) => (<Box component="img" key={attachment} alt={attachment} src={attachment} sx={{ width: 64, height: 64, borderRadius: 1.5 }}/>))}
        </Stack>)}
    ...
    return (<Stack spacing={2} direction={{
            xs: 'column',
            md: 'row',
