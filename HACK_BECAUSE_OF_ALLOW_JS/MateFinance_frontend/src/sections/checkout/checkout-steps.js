...
export default function CheckoutSteps({ steps, activeStep, sx, ...other }) {
    return (<Stepper alternativeLabel activeStep={activeStep} connector={<StepConnector />} sx={{
            mb: { xs: 3, md: 5 },
            ...sx,
        }} {...other}>
...
function StepIcon({ active, completed }) {
    return (<Stack alignItems="center" justifyContent="center" sx={{
      ...
      {completed ? (<Iconify icon="eva:checkmark-fill" sx={{ color: 'primary.main' }}/>) : (<Box sx={{
                width: 8,
                height: 8,
                borderRadius: '50%',
