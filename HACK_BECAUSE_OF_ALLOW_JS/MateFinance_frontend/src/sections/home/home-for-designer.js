...
export default function HomeForDesigner() {
    ...
    const renderDescription = (<Box sx={{ textAlign: { xs: 'center', md: 'unset' }, mt: { xs: 10, md: 20 } }}>
      ...
      <m.div variants={varFade().inUp}>
        <Button color="inherit" size="large" variant="contained" endIcon={<Iconify icon="eva:arrow-ios-forward-fill"/>} target="_blank" rel="noopener" href={paths.figma}>
          Figma Workspace
        </Button>
      </m.div>
    </Box>);
    const renderImg = (<Box component={m.img} src="/assets/images/home/for_designer.webp" variants={varFade().in} sx={{
            height: 1,
            width: 0.5,
            objectFit: 'cover',
            position: 'absolute',
