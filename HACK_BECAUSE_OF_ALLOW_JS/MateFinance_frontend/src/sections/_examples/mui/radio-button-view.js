...
export default function RadioButtonView() {
    ...
    return (<>
      <ComponentHero>
        <CustomBreadcrumbs heading="Radio Buttons" links={[
            {
                name: 'Components',
          ...
          <ComponentBlock title="Placement">
            <FormControl component="fieldset">
              <RadioGroup row defaultValue="top">
                {PLACEMENTS.map((placement) => (<FormControlLabel key={placement} value={placement} label={placement} labelPlacement={placement} control={<Radio size="medium"/>} sx={{ textTransform: 'capitalize' }}/>))}
              </RadioGroup>
            </FormControl>
          </ComponentBlock>
          ...
          <ComponentBlock title="Colors">
            <FormControl component="fieldset">
              <RadioGroup value={value} onChange={handleChange}>
                {COLORS.map((color) => (<FormControlLabel key={color} value={color} control={<Radio size="medium" color={color}/>} label={color} sx={{ textTransform: 'capitalize' }}/>))}

                <FormControlLabel disabled value="a8" control={<Radio size="medium" color="error"/>} label="Disabled"/>
              </RadioGroup>
            </FormControl>
          </ComponentBlock>
        </Masonry>
