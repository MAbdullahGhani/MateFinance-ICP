...
export default function RatingView() {
    ...
    return (<>
      <ComponentHero>
        <CustomBreadcrumbs heading="Rating" links={[
            {
                name: 'Components',
      ...
      <Container sx={{ my: 10 }}>
        <Masonry columns={{ xs: 1, sm: 2, md: 3 }} spacing={3}>
          ...
          <ComponentBlock title="Custom icon and color">
            <Rating name="customized-color" defaultValue={2} getLabelText={(ratingValue) => `${ratingValue} Heart${ratingValue !== 1 ? 's' : ''}`} precision={0.5} icon={<Iconify icon="solar:heart-bold"/>} emptyIcon={<Iconify icon="solar:heart-bold"/>} sx={{
            color: 'info.main',
          ...
          <ComponentBlock title="Custom icon set">
            <Rating name="customized-icons" defaultValue={2} getLabelText={(ratingValue) => customIcons[ratingValue].label} IconContainerComponent={IconContainer}/>
          </ComponentBlock>

          <ComponentBlock title="Hover feedback">
            <Rating name="hover-feedback" value={value} precision={0.5} onChange={(event, newValue) => {
            setValue(newValue);
