...
export default function PaginationView() {
    ...
    return (<>
      <ComponentHero>
        <CustomBreadcrumbs heading="Pagination" links={[
            {
                name: 'Components',
      ...
      <Container sx={{ my: 10 }}>
        <Masonry columns={{ xs: 1, md: 2 }} spacing={3}>
          ...
          <ComponentBlock title="Colors">
            {COLORS.map((color) => (<Pagination key={color} color={color} count={10} variant="text"/>))}

            {COLORS.map((color) => (<Pagination key={color} color={color} count={10} variant="outlined"/>))}

            {COLORS.map((color) => (<Pagination key={color} color={color} count={10} variant="soft"/>))}
          </ComponentBlock>

          <ComponentBlock title="Sizes">
            {SIZES.map((size) => (<Pagination count={10} key={size} size={size}/>))}
          </ComponentBlock>
