...
export default function SwitchView() {
    return (<>
      <ComponentHero>
        <CustomBreadcrumbs heading="Switch" links={[
            {
                name: 'Components',
      ...
      <Container sx={{ my: 10 }}>
        <Masonry columns={{ xs: 1, md: 2 }} spacing={3}>
          ...
          <ComponentBlock title="Placement">
            <FormGroup row>
              {PLACEMENTS.map((placement) => (<FormControlLabel key={placement} value={placement} label={placement} labelPlacement={placement} control={<Switch />} sx={{ textTransform: 'capitalize' }}/>))}
            </FormGroup>
          </ComponentBlock>
          ...
          <ComponentBlock title="Colors">
            <FormControl component="fieldset">
              <FormGroup>
                {COLORS.map((color) => (<FormControlLabel key={color} control={<Switch defaultChecked color={color}/>} label={color} sx={{ textTransform: 'capitalize' }}/>))}
