...
export default function AccordionView() {
    ...
    return (<>
      <ComponentHero>
        <CustomBreadcrumbs heading="Accordion" links={[
            {
                name: 'Components',
      ...
      <Container sx={{ my: 10 }}>
        <Stack spacing={5}>
          <ComponentBlock title="Simple" spacing={0}>
            {_accordions.map((accordion, index) => (<Accordion key={accordion.value} disabled={index === 3}>
          ...
          <ComponentBlock title="Controlled" spacing={0}>
            {_accordions.map((item, index) => (<Accordion key={item.value} disabled={index === 3} expanded={controlled === item.value} onChange={handleChangeControlled(item.value)}>
                <AccordionSummary expandIcon={<Iconify icon="eva:arrow-ios-downward-fill"/>}>
