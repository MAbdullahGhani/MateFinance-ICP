...
export default function ToggleButtons() {
    ...
    return (<Masonry columns={3} spacing={3}>
      ...
      <ComponentBlock title="Colors">
        {COLORS.map((color) => (<ToggleButtonGroup exclusive key={color} color={color} value={view} onChange={handleChange}>
            {viewContent}
          </ToggleButtonGroup>))}

        <Box sx={{ display: 'block', width: 1, height: 16 }}/>

        {COLORS.map((color) => (<ToggleButton key={color} color={color} value="check" selected={selected} onChange={() => {
      ...
      <ComponentBlock title="Vertical & Standalone buttons">
        ...
        <ToggleButton value="check" selected={selected} onChange={() => {
            setSelected(!selected);
        }}>
          <Iconify icon="eva:checkmark-fill"/>
        </ToggleButton>
      ...
      <ComponentBlock title="Sizes">
        {SIZES.map((size) => (<ToggleButton key={size} size={size} value="check">
            <Iconify icon="eva:checkmark-fill"/>
          </ToggleButton>))}

        {SIZES.map((size) => (<ToggleButtonGroup exclusive key={size} size={size} value={alignment} onChange={handleAlignment}>
            {alignContent}
          </ToggleButtonGroup>))}
      </ComponentBlock>
