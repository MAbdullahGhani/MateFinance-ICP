...
export default function BadgeView() {
    return (<>
      <ComponentHero>
        <CustomBreadcrumbs heading="Badge" links={[
            {
                name: 'Components',
      ...
      <Container sx={{ my: 10 }}>
        <Masonry columns={{ xs: 1, md: 2 }} spacing={3}>
          <ComponentBlock title="Basic">
            {COLORS.map((color) => (<Badge key={color} badgeContent={4} color={color}>
                <Iconify icon="fluent:mail-24-filled" width={24}/>
              </Badge>))}
          </ComponentBlock>

          <ComponentBlock title="Maximum value">
            <Badge badgeContent={99} color="error" children={<Iconify icon="fluent:mail-24-filled" width={24}/>}/>
            <Badge badgeContent={100} color="error" children={<Iconify icon="fluent:mail-24-filled" width={24}/>}/>
            <Badge badgeContent={1000} max={999} color="error" children={<Iconify icon="fluent:mail-24-filled" width={24}/>}/>
          </ComponentBlock>
          ...
          <ComponentBlock title="Status">
            {STATUS.map((status) => (<Badge key={status} variant={status} anchorOrigin={{ vertical: 'bottom', horizontal: 'right' }}>
                <Box sx={{
                width: 40,
                height: 40,
                borderRadius: '50%',
