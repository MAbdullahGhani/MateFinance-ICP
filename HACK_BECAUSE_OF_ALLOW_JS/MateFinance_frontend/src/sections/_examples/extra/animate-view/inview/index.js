...
export default function Inview() {
    ...
    return (<Card sx={{ p: 3 }}>
      <Grid container sx={{ mb: 3 }}>
        <Grid xs={12} md={9}>
          <Toolbar isText={text.value} isMulti={multi.value} onChangeText={text.onToggle} onChangeMulti={multi.onToggle} onRefresh={handleRefresh}/>
        </Grid>
      </Grid>

      <Grid container spacing={3}>
        <Grid xs={12} md={9}>
          <ContainerView key={count} isText={text.value} isMulti={multi.value} selectVariant={selectVariant}/>
        </Grid>
        <Grid xs={12} md={3}>
