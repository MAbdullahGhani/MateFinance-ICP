...
export default function CarouselCenterMode({ data }) {
    ...
    return (<Box sx={{
      ...
      <CarouselArrows filled icon="noto:rightwards-hand" onNext={carousel.onNext} onPrev={carousel.onPrev}>
        <Carousel ref={carousel.carouselRef} {...carousel.carouselSettings}>
          {data.map((item) => (<Box key={item.id} sx={{ px: 1 }}>
...
function CarouselItem({ item }) {
    ...
    return (<Paper sx={{
      ...
      <CardContent sx={{
        ...
        <Link color="inherit" variant="overline" sx={{
            opacity: 0.72,
            alignItems: 'center',
            display: 'inline-flex',
