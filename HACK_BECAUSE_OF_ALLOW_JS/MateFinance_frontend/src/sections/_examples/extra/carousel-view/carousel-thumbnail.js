...
export default function CarouselThumbnail({ data }) {
    ...
    const renderLargeImg = (<Box sx={{
      ...
      <Carousel {...carouselLarge.carouselSettings} asNavFor={carouselThumb.nav} ref={carouselLarge.carouselRef}>
        {data.map((item) => (<Image key={item.id} alt={item.title} src={item.coverUrl} ratio="16/9"/>))}
      </Carousel>

      <CarouselArrowIndex index={carouselLarge.currentIndex} total={data.length} onNext={carouselThumb.onNext} onPrev={carouselThumb.onPrev}/>
    </Box>);
    const renderThumbnails = (<StyledThumbnailsContainer length={data.length}>
      <Carousel {...carouselThumb.carouselSettings} asNavFor={carouselLarge.nav} ref={carouselThumb.carouselRef}>
        {data.map((item, index) => (<Box key={item.id} sx={{ px: 0.5 }}>
            <Avatar variant="rounded" alt={item.title} src={item.coverUrl} sx={{
                width: THUMB_SIZE,
                height: THUMB_SIZE,
                opacity: 0.48,
                cursor: 'pointer',
