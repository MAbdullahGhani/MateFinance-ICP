...
import { _mock, _ecommerceNewProducts, _ecommerceBestSalesman, _ecommerceSalesOverview, _ecommerceLatestProducts, } from 'src/_mock';
...
export default function WalktourView() {
        ...
        steps: [
            {
                ...
                title: 'Step 1',
                disableBeacon: true,
                content: (<Typography sx={{ color: 'text.secondary' }}>
            ...
            <TextField variant="filled" fullWidth label="Email" placeholder="example@gmail.com" InputLabelProps={{ shrink: true }} InputProps={{
            ...
            <Stack component={Paper} variant="outlined" divider={<Divider sx={{ borderStyle: 'dashed' }}/>}>
              {[
                    ...
                    ].map((option) => (<ListItem key={option.label}>
                  <Iconify width={26} icon={option.icon} sx={{ color: 'text.secondary', mr: 2 }}/>
                  <Box component="span" id={`switch-list-label-${option.label}`} sx={{ typography: 'subtitle2', flexGrow: 1 }}>
                    {option.label}
                  </Box>
                  <Switch color="default" defaultChecked={option.defaultChecked} edge="end" inputProps={{
            ...
            <Box sx={{
              ...
              {[...Array(6)].map((_, index) => (<Box key={index} component="img" alt="cover" src={_mock.image.cover(index)} sx={{ borderRadius: 1 }}/>))}
            </Box>
          </Stack>),
            },
        ],
    ...
    return (<>
      <Walktour continuous showProgress showSkipButton disableOverlayClose steps={walktour.steps} run={walktour.run} callback={walktour.onCallback} getHelpers={walktour.setHelpers} scrollDuration={500}/>

      <ComponentHero>
        <CustomBreadcrumbs heading="Walktour" links={[
            {
                name: 'Components',
      ...
      <Container sx={{ my: 10 }}>
        <Stack alignItems="flex-end" sx={{ mb: 5 }}>
          <Button size="large" variant="outlined" onClick={() => window.location.reload()} startIcon={<Iconify icon="solar:restart-bold"/>}>
            Reload
          </Button>
        </Stack>
        ...
        <Grid container spacing={3}>
          <Grid xs={12} md={8}>
            <EcommerceWelcome id="demo__1" title={`Congratulations! \n Jaydon Frankie`} description="Best seller of the month You have done 57.6% more sales today." img={<MotivationIllustration />} action={<Button variant="contained" color="primary">
                  Go Now
                </Button>}/>
          </Grid>
          ...
          <Grid xs={12} md={4}>
            <EcommerceWidgetSummary title="Product Sold" percent={2.6} total={765} chart={{
            series: [22, 8, 35, 50, 82, 84, 77, 12, 87, 43],
        }}/>
          </Grid>

          <Grid xs={12} md={4}>
            <EcommerceWidgetSummary id="demo__3" title="Total Balance" percent={-0.1} total={18765} chart={{
          ...
          <Grid xs={12} md={4}>
            <EcommerceWidgetSummary title="Sales Profit" percent={0.6} total={4876} chart={{
          ...
          <Grid xs={12} md={6} lg={4}>
            <EcommerceSaleByGender title="Sale By Gender" total={2324} chart={{
            series: [
                { label: 'Mens', value: 44 },
          ...
          <Grid xs={12} md={6} lg={8}>
            <EcommerceYearlySales id="demo__4" title="Yearly Sales" subheader="(+43%) than last year" chart={{
            categories: [
                'Jan',
                'Feb',
                'Mar',
          ...
          <Grid xs={12} md={6} lg={8}>
            <EcommerceBestSalesman title="Best Salesman" tableData={_ecommerceBestSalesman} tableLabels={[
          ...
          <Grid xs={12} md={6} lg={4}>
            <EcommerceLatestProducts id="demo__5" title="Latest Products" list={_ecommerceLatestProducts}/>
          </Grid>
        </Grid>
      </Container>
    </>);
