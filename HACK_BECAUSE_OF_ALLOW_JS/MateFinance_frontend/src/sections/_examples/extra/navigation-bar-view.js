...
export default function NavigationBarView() {
    ...
    const renderNavBasic = (<>
      <Stack spacing={2}>
        ...
        <Stack direction="row" alignItems="center" justifyContent="space-between" sx={{
          ...
          <NavBasicDesktop data={BASIC_NAV_ITEMS} slotProps={{
            rootItem: {
                fontSize: 15,
      ...
      <Drawer open={mobileOpen.value} onClose={mobileOpen.onFalse} PaperProps={{
            sx: {
                width: 260,
            },
        }}>
        <Logo sx={{ m: 2 }}/>
        <NavBasicMobile data={BASIC_NAV_ITEMS} slotProps={{
            rootItem: {
                fontSize: 15,
                fontFamily: (theme) => theme.typography.fontSecondaryFamily,
                '& .icon': {
    ...
    const renderNavVertical = (<Stack spacing={2}>
      <Typography variant="h6"> Nav Vertical </Typography>

      <NavSectionVertical data={NAV_ITEMS} sx={{
            py: 2,
            borderRadius: 2,
            bgcolor: 'background.paper',
            boxShadow: (theme) => theme.customShadows.z20,
    ...
    const renderNavMini = (<Stack spacing={2}>
      <Typography variant="h6"> Nav Mini </Typography>

      <NavSectionMini data={NAV_ITEMS} sx={{
            py: 0.5,
            borderRadius: 2,
            bgcolor: 'background.paper',
            boxShadow: (theme) => theme.customShadows.z20,
    ...
    const renderNavHorizontal = (<Stack spacing={2}>
    ...
    return (<>
      <ComponentHero>
        <CustomBreadcrumbs heading="Navigation Bar" links={[
            {
                name: 'Components',
      ...
      <Container sx={{ my: 10 }}>
        ...
        <Stack sx={{
          ...
          <Stack direction="row" spacing={10} sx={{ mt: 5 }}>
            ...
            <ControlsPanel config={config} onChangeConfig={handleChangeConfig} canReset={canReset} onReset={handleReset}/>
          </Stack>
        </Stack>
      </Container>
    </>);
...
function ControlsPanel({ config, onChangeConfig, canReset, onReset }) {
    return (<Stack component={Paper} spacing={3} variant="outlined" sx={{ p: 3, borderRadius: 2 }}>
      <Stack direction="row" alignItems="center" sx={{ width: 1 }}>
        <Typography variant="h6" sx={{ flexGrow: 1 }}>
          ControlsPanel
        </Typography>

        {canReset && (<IconButton onClick={onReset}>
      ...
      <Stack spacing={1}>
        ...
        <ToggleButtonGroup exclusive size="small" value={config.gap} onChange={(event, newValue) => {
            if (newValue !== null) {
                onChangeConfig('gap', newValue);
            }
        }}>
          {[4, 8, 16, 40].map((i) => (<ToggleButton key={i} value={i} sx={{ width: 1 }}>
              {i}
            </ToggleButton>))}
        </ToggleButtonGroup>
      </Stack>
      ...
      <Stack spacing={1}>
        ...
        <ToggleButtonGroup exclusive size="small" value={config.icon} onChange={(event, newValue) => {
            if (newValue !== null) {
                onChangeConfig('icon', newValue);
            }
        }}>
          {[0, 16, 20, 24, 40].map((i) => (<ToggleButton key={i} value={i} sx={{ width: 1 }}>
              {i}
            </ToggleButton>))}
        </ToggleButtonGroup>
      </Stack>
      ...
      <Stack spacing={1}>
        ...
        <ToggleButtonGroup exclusive size="small" value={config.radius} onChange={(event, newValue) => {
            if (newValue !== null) {
                onChangeConfig('radius', newValue);
            }
        }}>
          {[0, 8, 16, 40].map((i) => (<ToggleButton key={i} value={i} sx={{ width: 1 }}>
              {i}
            </ToggleButton>))}
        </ToggleButtonGroup>
      </Stack>
      ...
      <Stack spacing={1}>
        ...
        <ToggleButtonGroup exclusive size="small" value={config.currentRole} onChange={(event, newValue) => {
            if (newValue !== null) {
                onChangeConfig('currentRole', newValue);
            }
        }}>
          {['admin', 'user'].map((i) => (<ToggleButton key={i} value={i} sx={{ width: 1 }}>
              {i}
            </ToggleButton>))}
        </ToggleButtonGroup>
      </Stack>
      ...
      <Stack spacing={1}>
        ...
        <ToggleButtonGroup exclusive size="small" value={config.rootItemHeight} onChange={(event, newValue) => {
            if (newValue !== null) {
                onChangeConfig('rootItemHeight', newValue);
            }
        }}>
          {[36, 44, 64, 80].map((i) => (<ToggleButton key={i} value={i} sx={{ width: 1 }}>
              {i}
            </ToggleButton>))}
        </ToggleButtonGroup>
      </Stack>
      ...
      <Stack spacing={1}>
        ...
        <ToggleButtonGroup exclusive size="small" value={config.subItemHeight} onChange={(event, newValue) => {
            if (newValue !== null) {
      ...
      {/* Padding */}
      <TextField label="Item Padding" value={config.padding || ''} onChange={(event) => onChangeConfig('padding', event.target.value)}/>

      <FormControlLabel control={<Switch checked={config.hiddenLabel} onClick={() => onChangeConfig('hiddenLabel', !config.hiddenLabel)}/>} label="Hidden Label"/>

      <FormControlLabel control={<Switch checked={config.hiddenSubheader} onClick={() => onChangeConfig('hiddenSubheader', !config.hiddenSubheader)}/>} label="Hidden Subheader"/>
    </Stack>);
}
