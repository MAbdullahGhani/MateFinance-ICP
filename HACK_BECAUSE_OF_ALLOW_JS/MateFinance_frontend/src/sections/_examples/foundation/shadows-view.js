...
export default function ShadowsView() {
    ...
    return (<>
      <ComponentHero>
        <CustomBreadcrumbs heading="Shadows" links={[
            {
                name: 'Components',
      ...
      <Container sx={{ my: 10 }}>
        <Stack spacing={5}>
          <ComponentBlock title="System">
            {systemShadows.map((shadow, index) => (<ShadowCard key={shadow} title={`z${index + 1}`} sx={{ boxShadow: shadow }}/>))}
          </ComponentBlock>

          <ComponentBlock title="Customs">
            {customShadows.map((shadow) => (<ShadowCard key={shadow[0]} title={shadow[0]} sx={{ boxShadow: shadow[1] }}/>))}
          </ComponentBlock>

          <ComponentBlock title="Colors">
            {colorShadows.map((color) => (<ShadowCard key={color} title={color} sx={{
