...
export default function TypographyView() {
    return (<>
      <ComponentHero>
        <CustomBreadcrumbs heading="Typography" links={[
            {
                name: 'Components',
      ...
      <Container sx={{ my: 10 }}>
        <Grid container rowSpacing={5}>
          ...
          <Grid xs={12} md={9}>
            <Stack spacing={3}>
              {TYPOGRAPHYS.map((font) => (<BlockVariant key={font.variant} font={font}/>))}
            </Stack>
          </Grid>
          ...
          <Grid xs={12} md={9}>
            <Stack spacing={3}>
              {['primary', 'secondary', 'disabled'].map((color) => (<Paper key={color} variant="outlined" sx={{ p: 3, borderRadius: 1 }}>
              ...
              {['primary', 'secondary', 'info', 'warning', 'error'].map((color) => (<Paper key={color} variant="outlined" sx={{ p: 3, borderRadius: 1 }}>
...
function BlockVariant({ font }) {
    ...
    return (<Paper variant="outlined" sx={{ p: 3, borderRadius: 1 }}>
