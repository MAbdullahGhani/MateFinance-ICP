...
export default function ColorsView() {
    ...
    return (<>
      <ComponentHero>
        <CustomBreadcrumbs heading="Color" links={[
            {
                name: 'Components',
      ...
      <Container sx={{ my: 10 }}>
        <Box gap={3} display="grid" gridTemplateColumns={{
          ...
          {PALETTE.map((color) => (<Stack key={color} sx={{
                borderRadius: 1,
                overflow: 'hidden',
              ...
              <Stack spacing={1} sx={{ p: 2.5 }}>
                {VARIATIONS.map((variation) => (<ColorCard key={variation} variation={variation} hexColor={theme.palette[color][variation]} onCopy={() => onCopy(theme.palette[color][variation])}/>))}
              </Stack>
            </Stack>))}
          ...
          <Stack sx={{
            ...
            <Stack spacing={1} sx={{ p: 2.5 }}>
              {GREY.map((variation) => (<ColorCard key={variation} variation={variation} hexColor={theme.palette.grey[variation]} onCopy={() => onCopy(theme.palette.grey[variation])}/>))}
            </Stack>
          </Stack>
        </Box>
      </Container>
...
function ColorCard({ hexColor, variation, onCopy }) {
    return (<Stack spacing={1} onClick={onCopy} sx={{
            px: 2,
            py: 1.5,
            borderRadius: 1,
            cursor: 'pointer',
      ...
      <Box gap={2.5} display="grid" gridTemplateColumns="repeat(2, 1fr)" sx={{ typography: 'caption' }}>
        <Box>
