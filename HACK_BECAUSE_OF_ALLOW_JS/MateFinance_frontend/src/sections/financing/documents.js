...
// @mui
import { Box, Button, Dialog, DialogActions, DialogContent, DialogTitle, Divider, MenuItem, Stack, TextField, Typography, } from '@mui/material';
// @template
...
export default function DocumentList({ userId, invoiceId }) {
    ...
    const { user: { documents }, } = useSelector((state) => state);
    const [docs, setDocs] = useState([]);
    const { userFinance: { organizationType }, } = useSelector((state) => state.user);
    useEffect(() => {
        loading.onTrue();
    ...
    const handleAttachmentNameChange = (event) => {
        ...
        console.log('🚀 ~ file: documents.jsx:90 ~ handleAttachmentNameChange ~  name, value:', name, value);
    ...
    return (<>
      ...
      <Box sx={{ mt: '60px', mb: '-35px' }}>
        ...
        }} onDrop={handleDrop} placeholder={<Stack spacing={0.5} alignItems="center">
              <Iconify icon="eva:cloud-upload-fill" width={40}/>
              <Typography variant="body2">Upload Attachments</Typography>
            </Stack>} sx={{ w: 1, flexGrow: 1, height: 'auto', py: 2.5, mb: 3 }}/>
      </Box>
      {/* Dialoge box for Filename and description */}

      <Dialog open={fileDetail.open} maxWidth="xs" onClose={() => setFileDetail((prev) => ({ file: '', fileTitle: '', fileDiscription: '', open: false }))}>
        <DialogTitle>File Detail</DialogTitle>
        <DialogContent>
          <TextField sx={{ mt: 2 }} select size="small" fullWidth value={fileDetail.docType} label="Document Type" name="docType" onChange={(event) => handleAttachmentNameChange(event)}>
            {docs?.length > 0 &&
            docs?.map((doc) => (<MenuItem key={doc} value={doc?.docType}>
                  {doc?.docType} {doc?.required ? '*' : ''}
                </MenuItem>))}
          </TextField>
          <TextField sx={{ mt: 2 }} fullWidth autoFocus onChange={(event) => handleAttachmentNameChange(event)} value={fileDetail?.fileTitle} name="fileTitle" label="Attachment Title" placeholder="" size="small" className="customTextField"/>

          <TextField sx={{ mt: 2 }} fullWidth onChange={(event) => handleAttachmentNameChange(event)} value={fileDetail?.fileDiscription} name="fileDiscription" label="Attachment Description" placeholder="" size="small" className="customTextField"/>
        </DialogContent>
        <DialogActions>
          <Button variant="contained" color="primary" onClick={handleUploadAttachment}>
            Upload
          </Button>
          <Button variant="contained" color="inherit" onClick={() => setFileDetail((prev) => ({
            file: '',
            fileTitle: '',
            fileDiscription: '',
            open: false,
