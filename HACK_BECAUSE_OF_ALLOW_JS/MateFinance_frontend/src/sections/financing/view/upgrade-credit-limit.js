...
export default function UpgradeCreditLimit() {
    // *redux
    const { auth: { userData: { userId }, }, user: { kpi, userFinance: { creditScore }, }, } = useSelector((state) => state);
    ...
    return (<Container maxWidth="lg">
      <CustomBreadcrumbs heading={t(`${lang.sidebar}.upgradeCreditLimit`)} subHeading={t(`${lang.subheader}.UpgradeCreditLimit`)} links={[
            {
      ...
      <Divider sx={{ my: 2 }}/>
      {/* # */}
      <UpgradeCreditScore creditScore={creditScore} creditScoreCriteriaList={creditScoreCriteriaList}/>
    </Container>);
}
