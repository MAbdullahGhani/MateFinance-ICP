        ...
        <TableContainer sx={{ position: 'relative', overflow: 'unset' }}>
          ...
          <Scrollbar>
            <Table size={table.dense ? 'small' : 'medium'} sx={{ minWidth: 800 }}>
              <TableHeadCustom order={table.order} orderBy={table.orderBy} headLabel={TABLE_HEAD} rowCount={tableData.length} numSelected={table.selected.length}/>

              <TableBody>
                {dataFiltered
            .slice(table.page * table.rowsPerPage, table.page * table.rowsPerPage + table.rowsPerPage)
            .map((row) => (<TableRow hover key={row.name}>
                ...
                : `${row.filename?.split(0, 1)}.${row.filename?.split('.').pop()}`}
                      ...
                      <TableCell align="center">
                        <IconButton onClick={() => window.open(`${userAttachments.url}${row.path}${userAttachments.token}`)
