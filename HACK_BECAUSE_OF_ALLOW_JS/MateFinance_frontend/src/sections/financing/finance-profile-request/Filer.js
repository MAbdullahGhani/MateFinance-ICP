...
export default function Filer({ filer }) {
    const { auth: { userAttachments }, user: { userFinance: { filerDetial, creditScore }, }, } = useSelector((state) => state);
    ...
    //
    return (<div>
      <Drawer open={filer.value} onClose={filer.onFalse} anchor="right" slotProps={{
    ...
    disabled={filerDetial && creditScore?.filer?.action !== 'Correction'} onClick={handleUpload}>
            Save
          </LoadingButton>
