...
export default function Guarantor({ drawer }) {
    const { user: { userFinance: { guarantorDetails, creditScore }, }, } = useSelector((state) => state);
    const relationship1 = [
        { name: 'Mother' },
        { name: 'Father' },
        { name: 'Sister' },
    ...
    const { reset, handleSubmit, setValue, getValues, formState: { isSubmitting }, } = methods;
    ...
    const renderList = (<Scrollbar>
      ...
      <FormProvider methods={methods} onSubmit={handleSubmit(handleSave)}>
        <List disablePadding>
          <Box rowGap={2} columnGap={2} display="grid" gridTemplateColumns={{
            xs: 'repeat(1, 1fr)',
            sm: 'repeat(1, 1fr)',
        }} sx={{ my: 4, px: 3 }}>
            <Typography variant="h6">Guarantor Contact 1</Typography>
            <div>
              <RHFSelect name="relationShip1" label="Relationship">
                {relationship1.map((rel) => (<MenuItem value={rel.name} key={rel.name}>
                    {rel.name}
                  </MenuItem>))}
              </RHFSelect>
            </div>
            <div>
              <RHFTextField name="name1" label="Name"/>
            </div>
            <div>
              {' '}
              <CustomPhoneNumberField name="phone1" label="Mobile Number" value={phone1} onChange={(e) => setValue('phone1', e)} validerror={methods?.formState?.errors}/>
            </div>
            <div>
              {' '}
              <RHFTextField name="cnic1" type="number" label="CNIC"/>
            </div>
          </Box>
        </List>
        ...
        <List disablePadding>
          <Box rowGap={2} columnGap={2} display="grid" gridTemplateColumns={{
            ...
            <div>
              <RHFSelect name="relationShip2" label="Relationship">
                {relationship2.map((rel) => (<MenuItem value={rel.name} key={rel.name}>
                    {rel.name}
                  </MenuItem>))}
              </RHFSelect>
            </div>
            <div>
              <RHFTextField name="name2" label="Name"/>
            </div>
            <div>
              {' '}
              <CustomPhoneNumberField name="phone2" label="Mobile Number" value={phone2} onChange={(e) => setValue('phone2', e)} validerror={methods?.formState?.errors}/>
            </div>
            <div>
              {' '}
              <RHFTextField name="cnic2" type="number" label="CNIC"/>
            </div>
          </Box>
        </List>
    ...
    disabled={guarantorDetails?.guarantor1?.name && creditScore?.guarantor?.action !== 'Correction'}>
            Save
          </LoadingButton>
    ...
    return (<div>
      <Drawer open={drawer.value} onClose={drawer.onFalse} anchor="right" slotProps={{
