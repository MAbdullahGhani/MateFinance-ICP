    ...
    return (<div>
      <Grid container spacing={2}>
        <Grid item xs={12} md={6}>
          <UploadBox accept={{ 'image/jpeg': [], 'image/png': [] }} file={checkType(userInfo.cnicFrontImage) === 'base64' ||
            checkType(userInfo.cnicFrontImage) === 'empty'
            ? userInfo.cnicFrontImage
            : `${userAttachments.url}${userInfo.cnicFrontImage}${userAttachments.token}`} onDrop={handleDropFront} sx={{
            mb: 3,
            py: 2.5,
            //   width: '640px',
            height: '300px',
        ...
        <Grid item xs={12} md={6}>
          <UploadBox accept={{ 'image/jpeg': [], 'image/png': [] }} file={checkType(userInfo.cnicBackImage) === 'base64' ||
            checkType(userInfo.cnicBackImage) === 'empty'
            ? userInfo.cnicBackImage
            : `${userAttachments.url}${userInfo.cnicBackImage}${userAttachments.token}`} onDrop={handleDropBack} sx={{
            mb: 3,
            py: 2.5,
            height: '300px',
            borderRadius: 1.5,
