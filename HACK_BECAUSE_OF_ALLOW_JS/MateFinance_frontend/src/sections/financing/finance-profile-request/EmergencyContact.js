...
export default function EmergencyContact({ action, completed, userFinance }) {
    const BasicInfoSchema = Yup.object().shape({
        ...
        contactPhone1: Yup.string().test('contactPhone1', 'Invalid Phone Number', (value) => isValidPhoneNumber(value)),
    ...
    return (<Box>
      <FormProvider methods={methods} onSubmit={handleSubmit(handleSave)}>
        ...
        <Grid container spacing={1} sx={{ p: 1.5 }}>
          ...
          <Grid item xs={12} sm={4}>
            <CustomPhoneNumberField name="contactPhone1" label="Mobile Number" value={phone1} onChange={(e) => setValue('contactPhone1', e)} validerror={methods?.formState?.errors}/>
          </Grid>
        </Grid>
        ...
        <Grid container spacing={1} sx={{ p: 1.5 }}>
          ...
          <Grid item xs={12} sm={4}>
            <CustomPhoneNumberField name="contactPhone2" label="Mobile Number" value={phone2} onChange={(e) => setValue('contactPhone2', e)} validerror={methods?.formState?.errors}/>
          </Grid>
        </Grid>
