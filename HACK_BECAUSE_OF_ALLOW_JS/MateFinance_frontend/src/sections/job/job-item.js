    ...
    return (<>
      <Card>
        ...
        <Stack sx={{ p: 3, pb: 2 }}>
          ...
          <ListItemText sx={{ mb: 1 }} primary={<Link component={RouterLink} href={paths.dashboard.job.details(id)} color="inherit">
                {title}
              </Link>} secondary={`Posted date: ${fDate(createdAt)}`} primaryTypographyProps={{
        ...
        ].map((item) => (<Stack key={item.label} spacing={0.5} flexShrink={0} direction="row" alignItems="center" sx={{ color: 'text.disabled', minWidth: 0 }}>
              {item.icon}
