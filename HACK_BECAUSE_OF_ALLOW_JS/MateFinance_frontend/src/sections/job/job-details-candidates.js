...
export default function JobDetailsCandidates({ candidates }) {
    return (<Box gap={3} display="grid" gridTemplateColumns={{
      ...
      {candidates.map((candidate) => (<Stack component={Card} direction="row" spacing={2} key={candidate.id} sx={{ p: 3 }}>
          ...
          <Stack spacing={2}>
            <ListItemText primary={candidate.name} secondary={candidate.role} secondaryTypographyProps={{
                mt: 0.5,
                component: 'span',
                typography: 'caption',
                color: 'text.disabled',
