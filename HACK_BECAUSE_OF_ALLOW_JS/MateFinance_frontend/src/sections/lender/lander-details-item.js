...
import { ATTACHMENT_TOKEN, ATTACHMENT_URL, IM_HOST_ARB_EXPLORER, IM_HOST_EXPLORER, SMART_CON_URL, } from 'src/config-global';
...
import { haqInvoiceMateContractABI, haqInvoiceMateContractAddress, haqJsonURL, haqqChainId, haqWalletPrivateKey, } from 'src/haqchain-config';
...
export default function LenderDetailsItems({ status, statusOptions, lender, getLenderDetails }) {
    ...
    async function whiteListLenderWallet(id, walletId, status) {
        ...
        const arbInvoiceMateContract = new ethers.Contract(arbitrum?.InvoiceMateContractAddress, arbitrum?.InvoiceMateContractABI, arbDeployer);
        try {
            ...
            setLoading(false);
            enqueueSnackbar(`Wallet has been ${status === 'whiteList' ? 'whitelisted' : 'blacklisted'} successfully`);
            getLenderDetails();
        }
        catch (error) {
            setLoading(false);
    ...
    const getTransaction = async () => {
        setLoading(false);
        try {
            const response = await axios.get(endpoints.app.getTransactionLender + `?lenderId=${lender._id}`);
    ...
    return (<>
      ...
      <Box component={Paper} sx={{
        ...
        <Box sx={{
          ...
          <Box>
            <Avatar src={ATTACHMENT_URL + lender?.profilePicture[0]?.attachmentURL + ATTACHMENT_TOKEN} variant="circular" sx={{ width: 120, height: 120, mr: 2, mt: -10, zindex: 1000000 }}/>
            <ListItemText sx={{
            mt: 0.5,
            mx: 0.5,
        }} primary={lender?.firstName + ' ' + lender?.lastName} secondary={lender?.email} primaryTypographyProps={{
            typography: 'h4',
          ...
          <Box>
            <Label variant="soft" color={(status === 'approve' && 'success') ||
        ...
        <Stack sx={{
          ...
          <Box key={1} direction="row" alignItems="center" sx={{
            py: 2,
            // minWidth: 640,
            ...
            <Box display="flex" flexDirection="row" gap={2}>
              ...
              <List>
                <ListItem>
                  ...
                  <ListItemText primary="Email Verification" secondary={lender?.isActive ? 'Active' : 'No'}/>
                </ListItem>
              </List>
            </Box>
          </Scrollbar>
              ...
              <Scrollbar>
                <Box rowGap={5} p={2}>
                  <Stack sx={{ typography: 'body2' }} direction="row" gap={4}>
                    {lender?.identityDoc?.length > 0 ? (<Box sx={{
                m: 0.5,
                width: 100,
                cursor: 'pointer',
            }} onClick={() => window.open(ATTACHMENT_URL +
                ...
                ATTACHMENT_TOKEN)}>
                    ...
                    {/*  */}
                    {lender?.additionalDocument?.length > 0 ? (<Box sx={{
                m: 0.5,
                width: 100,
                cursor: 'pointer',
            }} onClick={() => window.open(ATTACHMENT_URL +
                ...
                color: 'text.disabled',
                          ...
                          <TableCell>
                            ...
                            <Tooltip title="WhiteList Wallet">
                              <IconButton onClick={() => handleUpdateStatus(item?._id, item?.walletAddress, 'whiteList')} disabled={item?.walletStatus === 'whiteList'}>
                                <Iconify icon="quill:paper"/>
                              </IconButton>
                            </Tooltip>
                            <Tooltip title="BlackList Wallet">
                              <IconButton onClick={() => handleUpdateStatus(item?._id, item?.walletAddress, 'blackList')} disabled={item?.walletStatus === 'blackList'}>
