...
export default function LenderRequestList() {
    ...
    const getLendingRequests = async () => {
        setLoading(true);
        try {
            const response = await axios.get(`${endpoints.app.getAllLendingReq}?start=${table.page}&end=${table.rowsPerPage}&search=${filters.name}&status=${filters.status}`);
    ...
    const handleFilter = useCallback((event, newValue) => {
    ...
    return (<>
      ...
      <Container maxWidth={settings.themeStretch ? false : 'lg'}>
        ...
        }} value={filters.name} onChange={handleFilter} placeholder="Search Lender & borrower..." InputProps={{
        ...
        >
          ...
          <TableContainer sx={{ position: 'relative', overflow: 'unset' }}>
            ...
            .map((row) => (<LenderRequestTableRow key={row.id} row={row} selected={table.selected.includes(row.id)} onSelectRow={() => table.onSelectRow(row.id)} onDeleteRow={() => handleDeleteRow(row.id)} onViewRow={() => handleViewRow(row?.dealData?._id)}/>))}
