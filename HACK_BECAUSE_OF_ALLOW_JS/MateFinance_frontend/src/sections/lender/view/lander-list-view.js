...
import LenderTableToolbar from '../lander-table-toolbar';
import LenderTableFiltersResult from '../lander-table-filters-result';
import LenderTableRow from '../lander-table-row';
...
const TABLE_HEAD = [
    { id: 'firstName', label: 'Borrower' },
    { id: 'status', label: 'Status' },
    { id: 'isActive', label: 'Email Verified' },
...
// ----------------------------------------------------------------------
export default function LenderListView() {
    ...
    const getLendersList = async () => {
        setLoading(true);
        try {
            const response = await axios.get(`${endpoints.app.getAllLenders}?start=${table.page}&end=${table.rowsPerPage}&search=${filters.name}&status=${filters.status}`);
            setPage(response?.totalNumOfItems);
            setTableData(response?.lender || []);
            setLoading(false);
        }
        catch (error) {
            setLoading(false);
    ...
    const handleViewRow = useCallback((id) => {
        router.push(paths.dashboard.lender.details(id));
    }, [router]);
    ...
    return (<>
      ...
      <CustomBreadcrumbs heading="Lenders" links={[
            {
                name: 'Dashboard',
      ...
      <Container maxWidth={settings.themeStretch ? false : 'lg'}>
        ...
        }} fullWidth value={filters.name} onChange={handleFilterName} placeholder="Search Lender..." InputProps={{
        ...
        <Card>
          <Tabs value={filters.status} onChange={handleFilterStatus} sx={{
            ...
            {STATUS_OPTIONS.map((tab) => (<Tab key={tab.value} iconPosition="end" value={tab.value} label={tab.label}/>))}
          </Tabs>
          ...
          <TableContainer sx={{ position: 'relative', overflow: 'unset' }}>
            ...
            <Scrollbar>
              <Table size={table.dense ? 'small' : 'medium'} sx={{ minWidth: 960 }}>
                ...
                <TableBody>
                  {dataFiltered.map((row) => (<LenderTableRow key={row._id} row={row} selected={table.selected.includes(row.id)} onSelectRow={() => table.onSelectRow(row.id)} onDeleteRow={() => handleDeleteRow(row.id)} onViewRow={() => handleViewRow(row._id)}/>))}
