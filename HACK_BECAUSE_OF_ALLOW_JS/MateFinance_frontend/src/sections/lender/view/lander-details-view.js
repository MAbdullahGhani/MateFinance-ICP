...
export default function LenderDetailsView({ id }) {
    ...
    return (<>
      ...
      <Container maxWidth={settings.themeStretch ? false : 'lg'}>
        <LenderDetailsToolbar backLink={paths.dashboard.lender.root} statusOptions={LENDER_STATUS_OPTIONS} status={lenderDetails?.status} createdAt={lenderDetails?.createdAt || new Date()} onChangeStatus={handleUpdateStatus}/>

        <Grid container spacing={3}>
          <Grid xs={12} md={12}>
            <Stack spacing={3} direction={{ md: 'column' }}>
              {lenderDetails && (<LenderDetailsItems status={lenderDetails?.status} statusOptions={LENDER_STATUS_OPTIONS} lender={lenderDetails} getLenderDetails={getLenderDetails}/>)}
            </Stack>
          </Grid>
