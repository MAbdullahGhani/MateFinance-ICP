...
export default function KanbanDetailsCommentList({ comments }) {
    ...
    return (<>
      <Stack spacing={3} flexGrow={1} sx={{
            py: 3,
            px: 2.5,
            bgcolor: 'background.neutral',
        }}>
        {comments.map((comment) => (<Stack key={comment.id} direction="row" spacing={2}>
            ...
            <Stack spacing={comment.messageType === 'image' ? 1 : 0.5} flexGrow={1}>
              ...
              {comment.messageType === 'image' ? (<Image alt={comment.message} src={comment.message} onClick={() => lightbox.onOpen(comment.message)} sx={{
                    borderRadius: 1.5,
                    cursor: 'pointer',
