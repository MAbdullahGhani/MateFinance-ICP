...
export default function KanbanDetailsPriority({ priority, onChangePriority }) {
    return (<Stack direction="row" flexWrap="wrap" spacing={1}>
      {['low', 'medium', 'hight'].map((option) => (<ButtonBase key={option} onClick={() => onChangePriority(option)} sx={{
                py: 0.5,
                pl: 0.75,
                pr: 1.25,
                fontSize: 12,
          ...
          <Iconify icon={(option === 'low' && 'solar:double-alt-arrow-down-bold-duotone') ||
