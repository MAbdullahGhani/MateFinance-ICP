...
export default function PostItem({ post, index }) {
    ...
    if (mdUp && latestPost) {
        return (<Card>
        <Avatar alt={author.name} src={author.avatarUrl} sx={{
                top: 24,
                left: 24,
                zIndex: 9,
                position: 'absolute',
        ...
        <PostContent title={title} createdAt={createdAt} totalViews={totalViews} totalShares={totalShares} totalComments={totalComments} index={index}/>

        <Image alt={title} src={coverUrl} overlay={alpha(theme.palette.grey[900], 0.48)} sx={{
                width: 1,
                height: 360,
            }}/>
      </Card>);
    ...
    return (<Card>
      ...
      <PostContent title={title} totalViews={totalViews} totalComments={totalComments} totalShares={totalShares} createdAt={createdAt}/>
    </Card>);
}
PostItem.propTypes = {
...
export function PostContent({ title, createdAt, totalViews, totalShares, totalComments, index }) {
    ...
    return (<CardContent sx={{
      ...
      <Stack spacing={1.5} direction="row" justifyContent="flex-end" sx={{
            mt: 3,
            typography: 'caption',
            color: 'text.disabled',
            // ...((latestPostLarge || latestPostSmall) && {
