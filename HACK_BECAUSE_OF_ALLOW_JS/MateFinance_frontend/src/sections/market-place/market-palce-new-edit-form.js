...
import FormProvider, { RHFCheckbox, RHFSelect, RHFTextField, RHFUpload, RHFUploadAvatar, } from 'src/components/hook-form';
...
export default function DealNewEditForm({ currentJob }) {
    ...
    const NewDealSchema = Yup.object().shape({
        ...
        kyiLink: Yup.string()
            .nullable()
            .test('is-valid-url-or-empty', 'Enter correct URL!', (value) => !value ||
    ...
    const { reset, control, watch, setValue, handleSubmit, formState: { isSubmitting }, } = methods;
    const values = watch();
    ...
    const getDeal = async () => {
        ...
        try {
            ...
            if (deal?.borrower?.logo.length > 0) {
                ...
                setValue('logo', ATTACHMENT_URL + deal?.borrower?.logo[0]?.attachmentPath + ATTACHMENT_TOKEN);
                setLogoUpload(file);
            }
        }
        catch (error) {
    ...
    const getInvoiceURI = async (data) => {
        try {
            ...
            const res = await axios.post(`https://betapbc.invoicemate.net/ipfs-api/upload`, edit === 'true' ? editPayload : payload);
    ...
    const storeBorrowerDetails = async (data) => {
        ...
        const invoiceMateContract = new ethers.Contract(_invoiceMateContract, check ? haqq?.InvoiceMateContractABI : arbitrum?.InvoiceMateContractABI, deployer);
        ...
        try {
            ...
            await txn.wait();
            const tx = await invoiceMateContract.connect(deployer).requestLoan(borrower, principleAmt, // principle
            convertDaysToMonths(data?.loanTenure), //loan term months
            invoiceUri, data?.apyRate);
            ...
            if (receipt.status === 1) {
                ...
                await axiosInstance.post(edit !== 'true' ? endpoints.app.createDeal : endpoints.app.updateDeal, {
                    ...payload,
                });
      ...
      <Grid xs={12} md={8}>
        <Card>
          ...
          <Stack spacing={3} sx={{ p: 3 }}>
            <Box columnGap={2} rowGap={3} display="grid" gridTemplateColumns={{
            xs: 'repeat(1, 1fr)',
            md: 'repeat(2, 1fr)',
        }}>
              <RHFTextField name="loanAmount" label="Loan Amount" type="number" disabled InputProps={{
            endAdornment: (<InputAdornment position="end">
              ...
              <RHFSelect name="loanTenure" label="Loan Tenure">
                {['15 Days', '30 Days', '45 Days', '60 Days', '75 Days', '90 Days', '120 Days'].map((deal) => (<MenuItem value={deal} key={deal}>
                      {deal}
                    </MenuItem>))}
              </RHFSelect>
              <RHFSelect name="settlementCycle" label="Settlement cycle">
                {['End of loan term'].map((deal) => (<MenuItem value={deal} key={deal}>
                    {deal}
                  </MenuItem>))}
              </RHFSelect>
              <RHFSelect name="dealExpiresIn" label="Deal Expires In">
                {['5 Days', '10 Days', '15 Days', '20 Days', '25 Days', '30 Days'].map((deal) => (<MenuItem value={deal} key={deal}>
                    {deal}
                  </MenuItem>))}
              </RHFSelect>
              <RHFTextField type="number" name="minimumFinancingAmt" label="Financing Amt. (Min)" InputProps={{
            endAdornment: (<InputAdornment position="end">
                      <small>USD</small>
                    </InputAdornment>),
        }}/>
              <RHFTextField type="number" name="maximumFinancingAmt" label="Financing Amt. (Max)" InputProps={{
            endAdornment: (<InputAdornment position="end">
                      <small>USD</small>
                    </InputAdornment>),
        }}/>
              <RHFTextField type="number" name="earlyWithdrawFee" label="Early Withdraw Fee" InputProps={{
            endAdornment: (<InputAdornment position="end">
                      <small>USD</small>
                    </InputAdornment>),
        }}/>
              <RHFSelect name="chain" label="Select Chain">
                {chains.map((chain) => (<MenuItem value={chain.value} key={chain.value}>
                    {chain.name}
                  </MenuItem>))}
              </RHFSelect>
              <RHFSelect name="liquidityPool" label="Liquidity Pool">
                {['High Risk Pool', 'Low Risk Pool'].map((pool) => (<MenuItem value={pool} key={pool}>
                    {pool}
                  </MenuItem>))}
              </RHFSelect>
            ...
            <RHFTextField name="loanSummary" label="Loan Summary" multiline rows={4} inputProps={{ maxLength: 500 }} InputProps={{
            endAdornment: (<div style={{ fontSize: '0.75rem', color: 'rgba(0, 0, 0, 0.54)', alignSelf: 'end' }}>
      ...
      <Grid xs={12} md={8}>
        <Card>
          ...
          <Stack spacing={3} sx={{ p: 3 }}>
            ...
            <RHFTextField name="loanStartDate" label="Loan Start Date" type="date" InputLabelProps={{ shrink: true }}/>
            {/* <RHFTextField
      ...
      <Grid xs={12} md={8}>
        <Card>
          ...
          <Stack spacing={3} sx={{ p: 3 }}>
            ...
            <RHFTextField name="websiteLink" label="Website Link" InputProps={{
            startAdornment: <InputAdornment position="start">https://</InputAdornment>,
        }}/>
            <RHFTextField name="linkedinLink" label="LinkedIn Link" InputProps={{
            startAdornment: <InputAdornment position="start">https://</InputAdornment>,
        }}/>
            <RHFTextField name="twitterLink" label="Twitter Link" InputProps={{
            startAdornment: <InputAdornment position="start">https://</InputAdornment>,
        }}/>
            <Stack flexDirection="column" spacing={2}>
              <small>Upload Logo</small>
              <Stack flexDirection="row" spacing={2}>
                <RHFUploadAvatar name="logo" maxSize={3145728} onDrop={(file) => handleDrop(file, 'logo')} onDelete={() => handleRemoveFile('logo')} disabled/>
                <RHFUpload name="logo" maxSize={3145728} onDrop={(file) => handleDrop(file, 'logo')} onDelete={() => handleRemoveFile('logo')}/>
                {/* {values.logo && (
          <Box
            sx={{
              m: 0.5,
    ...
    const renderRiskMitigation = (<>
      ...
      <Grid xs={12} md={8}>
        <Card>
          ...
          <Stack spacing={3} sx={{ p: 3 }}>
            <RHFTextField name="kyiLink" label="KYI Link" InputProps={{
            startAdornment: <InputAdornment position="start">https://</InputAdornment>,
            endAdornment: (<InputAdornment position="end">
                    <IconButton onClick={() => {
      ...
      <Grid xs={12} md={8}>
        <Card>
          ...
          <Stack spacing={3} sx={{ p: 3 }}>
            ...
            <RHFTextField name="underWLink" label="Website Link" InputProps={{
            startAdornment: <InputAdornment position="start">https://</InputAdornment>,
        }}/>
          </Stack>
        </Card>
    ...
    const renderActions = (<>
      ...
      {mdUp && values.chain !== 'Haqq Network' && <Grid md={6}/>}
      <Grid xs={12} md={6} sx={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end' }}>
    ...
    disabled={isSavingDraft}>
        ...
        <LoadingButton variant="contained" sx={{
            ...
            gap: 1,
            display: 'inline-flex',
        }} type="submit" name="createPool" 
    // onClick={handleCreatePool}
    loading={isCreatingPool} disabled={isCreatingPool}>
          <div style={{
            textAlign: 'center',
            color: 'white',
            fontSize: 16,
