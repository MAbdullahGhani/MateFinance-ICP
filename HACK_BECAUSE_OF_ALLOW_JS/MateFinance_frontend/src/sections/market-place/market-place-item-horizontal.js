...
export default function MarketPlaceItemHorizontal({ post, tab }) {
    ...
    const { borrower, closed, createdAt, financeRequest, lending, overview, repayment, riskMitigation, tokenized, status, Borrower, } = post;
    ...
    return (<>
      <Stack component={Card} direction="row" justifyContent="space-between" sx={{
            // ...bgGradient({
            //   direction: '135deg',
        ...
        <Stack sx={{
          ...
          <Stack direction="row" alignItems="center" justifyContent="space-between" sx={{ mb: 2 }}>
            <Label variant="soft" sx={{ background: statusColors(status?.toLowerCase()), color: 'white' }}>
          ...
          <Stack spacing={1}>
            <Box display="grid" alignItems="center" justifyContent="space-between" gridTemplateColumns={{
            xs: 'repeat(1, 1fr)',
            sm: 'repeat(2, 1fr)',
        }}>
              <Link color="inherit" component={RouterLink} href={!user
            ? paths.auth.jwt.details(post?._id)
            : `${paths.dashboard.marketPlace.details(post?._id)}${tab === 'my Deals' ? `?tab=myDeals` : ''}`}>
          ...
          <Stack spacing={1}>
            ...
            sm: 'repeat(2, 1fr)',
              ...
              <Stack direction="column">
                ...
                <TextMaxLine variant="caption">
                  {tab === 'lending' && (<TextMaxLine variant="caption">
            ...
            : tab !== 'lending' &&
              ...
              {status === 'draft' && (<Stack direction="column" alignItems="flex-start">
        ...
        {smUp && (<Box sx={{
            ...
            <Box sx={{
              ...
              <img style={{ border: '2px solid white', borderRadius: '10px' }} src={post?.isFirozaAPI
                ? '/assets/firoza.png'
                : post?.overview?.chain === 'Arbitrum Network'
                    ? '/assets/arbitrum.png'
                    : '/assets/haqq.png'}/>
            </Box>
            <Image alt={borrower?.name} src={borrower?.logo[0]?.attachmentPath
                ? ATTACHMENT_URL + borrower?.logo[0]?.attachmentPath + ATTACHMENT_TOKEN
                : '/logo/mate.finance_verti.png'} sx={{ height: 1, borderRadius: 1.5 }}/>
          </Box>)}
      </Stack>
      ...
      <CustomPopover open={popover.open} onClose={popover.onClose} arrow="bottom-center" sx={{ width: 140 }}>
        ...
        <MenuItem onClick={() => {
            popover.onClose();
            router.push(`${paths.dashboard.marketPlace.createMarketPlace}?edit=${true}&id=${post?._id}`);
        }}>
