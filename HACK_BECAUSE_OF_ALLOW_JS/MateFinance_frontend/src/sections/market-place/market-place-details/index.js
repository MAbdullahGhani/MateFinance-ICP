...
import { Card, Avatar, Grid, IconButton, Button, Typography, Box, Divider, CardHeader, ListItemText, Tooltip, CardContent, TextField, InputAdornment, } from '@mui/material';
...
const MarketPlaceDetails = () => {
    ...
    async function _depositFunds() {
        ...
        const invoiceMateContract = new ethers.Contract(haqq?.InvoiceMateContractAddress, haqq?.InvoiceMateContractABI, deployer);
        try {
            // HAQ
            const tx = await invoiceMateContract
                .connect(deployer)
                .approveLoanFiroza(deal?.borrower?.borrowerWallet, deal?.returnBlockChainId, deal?.overview?.liquidityPool === 'High Risk Pool' ? 1 : 0);
            await tx.wait();
    ...
    async function cancelLoan() {
        ...
        try {
            await InvoiceMateContract.connect(deployer).changeStateOfLoan(deal?.borrower?.borrowerWallet, deal?.returnBlockChainId, 4);
    ...
    const approve = async () => {
        try {
            setLoading(true);
            const wallet = new ethers.Wallet('b5f923b26452f72582b278045b921d64ed12cb9c98dbd4fa409b8206da1300b8');
            // Create ethers wallet instance using private key
    ...
    return (<>
      ...
      <Container maxWidth={settings.themeStretch ? false : 'lg'}>
        <Grid container>
          <Grid sm={12} md={user && user?.role === 1 ? 8 : 8} sx={{ p: 1 }}>
            {user?.role === 1 && deal?.status === 'draft' && (<Button sx={{ ml: 1 }} color="primary" variant="contained" size="small" onClick={() => router.push(`${paths.dashboard.marketPlace.createMarketPlace}?edit=${true}&id=${deal?._id}`)}>
            ...
            <Button sx={{ float: 'right' }} color="primary" variant="contained" onClick={() => {
            ...
            (deal?.status !== 'lending' &&
                ...
                deal?.status !== 'cancelled' &&
                deal?.status !== 'rejected' &&
                !deal?.isFirozaAPI ? (<Button sx={{ float: 'right' }} color="primary" variant="contained" onClick={() => authDialog.onTrue()}>
                  Invest Now
                </Button>) : (''))}
            {user?.role === 1 && deal?.status === 'expired' ? (<Button sx={{ float: 'right' }} color="primary" variant="contained" onClick={() => {
                extendDialog.onTrue();
                setExtendDays(0);
            }}>
                Extend Expiry
            ...
            deal?.status !== 'cancelled' &&
            deal?.status !== 'rejected' &&
            deal?.isFirozaAPI ? (<Button sx={{ float: 'right' }} color="primary" variant="contained" onClick={approveLoan}>
                    Transfer Funds
                  </Button>) : ('')}
                {user?.role === 1 && deal?.status === 'tokenized' ? (<Button sx={{ float: 'right' }} color="primary" variant="contained" onClick={cancelDeal}>
                    Cancel Deal
                  </Button>) : ('')}
                {user?.role === 1 && deal?.status === 'tokenized' ? (<Button sx={{ float: 'right' }} color="primary" variant="contained" onClick={rejectDeal}>
                    Reject Deal
                  </Button>) : ('')}
              </Stack>
            </Stack>
          ...
          {user && user?.role === 1 && (<Grid xs={12} md={4} sx={{ p: 1, mx: 'auto' }}>
          ...
          <CardContent>
            <Stack spacing={1}>
              ...
              <Stack flexDirection="row" spacing={2}>
                <Button variant="text" color="primary" size="small" endIcon={<Iconify icon="ion:arrow-redo-outline"/>} onClick={() => {
            ...
            if (websiteLink) {
                ...
                <Button variant="text" color="primary" size="small" endIcon={<Iconify icon="ion:arrow-redo-outline"/>} onClick={() => window.open(`${IM_HOST_UI}/auth/jwt/invoice_view/${deal?.Borrower?.loanRequestId}`)}>
                  Invoice
                </Button>
                <Button variant="text" color="primary" size="small" endIcon={<Iconify icon="ion:arrow-redo-outline"/>} onClick={() => {
            const { haqq, arbitrum } = user;
            if (deal?.overview?.chain === 'Haqq Network') {
                window.open(`${IM_HOST_EXPLORER}/address/${haqq?.InvoiceMateContractAddress}`);
            }
            else {
                window.open(`${IM_HOST_ARB_EXPLORER}/address/${arbitrum?.InvoiceMateContractAddress}`);
            }
        }}>
                  Smart Contract
                </Button>
        ...
        }} endIcon={<Iconify icon="ion:arrow-redo-outline"/>} disabled={!deal?.borrower?.linkedinLink}>
                  LinkedIn
                </Button>
        ...
        {extendView === 'true' && (<Card sx={{ p: 3, mt: 2 }}>
            <Typography variant="subtitle1" gutterBottom>
        ...
        {user?.role === 1 || (user?.role === 2 && tab === 'myDeals') ? (<Card sx={{ mt: 2 }}>
      ...
      <Dialog fullWidth maxWidth="xs" open={authDialog.value} onClose={onClose}>
        ...
        <DialogActions>
          <Button variant="outlined" color="inherit" onClick={() => router.push(paths.auth.jwt.login)}>
            Sign in
          </Button>
          <Button variant="outlined" color="inherit" onClick={() => router.push(paths.auth.jwt.register)}>
            Sign up
          </Button>
        </DialogActions>
      </Dialog>

      <Dialog fullWidth maxWidth="xs" open={extendDialog.value} onClose={() => extendDialog.onFalse()}>
        <DialogTitle sx={{ pb: 2 }}>{'Extend Deal Expiry'}</DialogTitle>
        <DialogContent sx={{ typography: 'body2' }}>
          <TextField fullWidth type={'number'} value={extendDays} onChange={(event) => {
      ...
      <Dialog maxWidth="sm" fullWidth open={commentsDialog.value} onClose={commentsDialog.onFalse}>
        ...
        <DialogContent>
          <Stack spacing={2}>
            <TextField margin="normal" id="internalComments" label="Internal Notes" type="text" fullWidth variant="outlined" multiline rows={3} value={state.internalComments} onChange={(e) => setState({ ...state, internalComments: e.target.value })}/>
            <TextField rows={3} margin="normal" id="externalComments" label="External Notes" type="text" fullWidth variant="outlined" multiline value={state.externalComments} onChange={(e) => setState({ ...state, externalComments: e.target.value })}/>
          </Stack>
        </DialogContent>
        <DialogActions>
          <Button onClick={state.action === 'cancel' ? cancelLoan : rejectLoan} color="primary" variant="contained">
            Submit
          </Button>
      ...
      <ConfirmDialog open={confirm.value} onClose={confirm.onFalse} title={state.action === 'cancel' ? 'Cancel Deal' : 'Reject Deal'} content={state.action === 'cancel'
            ? 'Are you sure want to cancel?'
            : 'Are you sure want to reject?'} action={<Button variant="contained" sx={{ background: 'red' }} onClick={() => {
