...
export default function FormAccountEmailDialog(props) {
    // props
    const { dialog, dialogTitle, userPaymentChannel, paymentChannelUpdate, setPaymentChannelUpdate } = props;
    // hooks
    ...
    const { control, reset, setValue, handleSubmit, formState: { isSubmitting, errors }, } = methods;
    ...
    return (<Dialog open={dialog.value} onClose={dialog.onFalse} maxWidth="xs" fullWidth>
      {paymentChannelUpdate?.accountNumber ? (<DialogTitle>Update {dialogTitle} account</DialogTitle>) : (<DialogTitle>Add new {dialogTitle} account</DialogTitle>)}
        ...
        <DialogContent>
          <Box sx={{ width: { sx: 200 } }}>
            ...
            <div className="w-100">
              ...
              <RHFTextField name="email" size="small" disabled={!!paymentChannelUpdate?.accountNumber}/>
            </div>
          </Box>
        </DialogContent>
